...
<div <?php echo $wrapper_attr; ?>>
	<div class="dp-pricing-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:32px;max-width:1100px;margin:0 auto;">
		...
		<div class="dp-pricing-card<?php echo $tier['featured'] ? ' dp-pricing-card--featured' : ''; ?>" style="background:var(--wp--preset--color--white);border:2px solid<?php echo $tier['featured'] ? 'var(--wp--preset--color--brand-blue)' : 'var(--wp--preset--color--card-border)'; ?>;border-radius:16px;padding:40px 32px;text-align:center;position:relative;<?php echo $tier['featured'] ? 'transform:scale(1.02);' : ''; ?>">
			<?php if ( $tier['featured'] ) : ?>
			<div style="position:absolute;top:-14px;left:50%;transform:translateX(-50%);background:var(--wp--preset--color--brand-blue);color:white;padding:4px 20px;border-radius:20px;font-size:13px;font-weight:700;letter-spacing:0.5px;">MOST POPULAR</div>
			<?php endif; ?>
			...
			<div style="font-size:32px;font-weight:900;color:var(--wp--preset--color--brand-blue);margin:0 0 8px;"><?php echo esc_html( $tier['price'] ); ?></div>
			...
			<a href="/contact/" style="display:block;padding:14px 24px;background:<?php echo $tier['featured'] ? 'var(--wp--preset--color--brand-blue)' : 'var(--wp--preset--color--brand-dark)'; ?>;color:white;text-decoration:none;border-radius:10px;font-weight:700;font-size:16px;transition:opacity 0.2s;" onmouseover="this.style.opacity='0.85'" onmouseout="this.style.opacity='1'">Get a Quote →</a>
		</div>
		<?php endforeach; ?>
	</div>
</div>
